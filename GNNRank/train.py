...
def evalutaion(logstr, score, A_torch, label_np, val_index, test_index, SavePred, save_path, split, identifier_str, flag):
    ...
    if label_np is not None:
        ...
        #print("pred_label[test_index]", pred_label[test_index])
        ...
        #print("pred_label[val_index]", pred_label[val_index])
    ...
    if flag == 'test':
       print("flag = ", flag)
       return logstr, upset_full, kendalltau_full, score
    else:
            ...
            for split in range(self.splits):
                ...
                if self.args.optimizer == 'Adam':
                    ...
                    opt = optim.SGD(model.parameters(), lr=self.args.lr,
                                    weight_decay=self.args.weight_decay,momentum=0.99)
                else:
                ...
                score = model.obtain_score_from_dist()
                #np.savetxt("Score_Dist.txt",score.detach().cpu().numpy())
                #np.savetxt("Test_index1.txt",self.pred_label)
                logstr, upset_full[0, split], kendalltau_full[0, split], score_dist_test = evalutaion(logstr, score, self.A_torch, self.label_np, val_index, test_index, self.args.SavePred, \
                    base_save_path, split, 'dist', 'test')
                    
                np.savetxt("Score_Dist.txt",score_dist_test.detach().cpu().numpy())

                score = model.obtain_score_from_innerproduct()
                #np.savetxt("Score_innerproduct.txt",score.detach().cpu().numpy())
                #np.savetxt("Test_index2.txt",test_index)
                logstr, upset_full[1, split], kendalltau_full[1, split], score_inner_test = evalutaion(logstr, score, self.A_torch, self.label_np, val_index, test_index, self.args.SavePred, \
                    base_save_path, split, 'innerproduct', 'test')

                np.savetxt("Score_innerproduct.txt",score_inner_test.detach().cpu().numpy())
                for ind, start_from in enumerate(['dist', 'innerproduct', 'baseline']):
                    score = model.obtain_score_from_proximal(start_from)
                    #np.savetxt("Score_proximal"+start_from+".txt",score.detach().cpu().numpy())
                    #np.savetxt("Test_index3.txt",tensor.numpy(test_index))
                    logstr, upset_full[2 + ind, split], kendalltau_full[2 + ind, split], score_proximal = evalutaion(logstr, score, self.A_torch, self.label_np, val_index, test_index, self.args.SavePred, \
                        base_save_path, split, 'proximal_'+start_from, 'test')
                    np.savetxt("Score_proximal"+start_from+".txt",score_proximal.detach().cpu().numpy())
                ...
                logstr, upset_full_latest[0, split], kendalltau_full_latest[0, split] = evalutaion(logstr, score, self.A_torch, self.label_np, val_index, test_index, self.args.SavePred, \
                    base_save_path, split, 'dist_latest','latest')
                score = model.obtain_score_from_innerproduct()
                logstr, upset_full_latest[1, split], kendalltau_full_latest[1, split] = evalutaion(logstr, score, self.A_torch, self.label_np, val_index, test_index, self.args.SavePred, \
                    base_save_path, split, 'innerproduct_latest','latest')
                for ind, start_from in enumerate(['dist', 'innerproduct', 'baseline']):
                    score = model.obtain_score_from_proximal(start_from)
                    logstr, upset_full_latest[2 + ind, split], kendalltau_full_latest[2 + ind, split] = evalutaion(logstr, score, self.A_torch, self.label_np, val_index, test_index, self.args.SavePred, \
                        base_save_path, split, 'proximal_'+start_from+'_latest', 'latest')
